...
class IgManagerFhirDal(private val igContext: IgContext, private val igManager: IgManager) :
  FhirDal {
  override fun read(id: IIdType): IBaseResource? = runBlocking {
    igManager.loadById(igContext, id.resourceType, id.idPart).firstOrNull()
  }
  ...
  override fun search(resourceType: String): Iterable<IBaseResource> = runBlocking {
    igManager.loadAll(igContext, resourceType)
  }

  override fun searchByUrl(resourceType: String, url: String): Iterable<IBaseResource> =
    runBlocking {
      igManager.loadAll(igContext, resourceType, url)
    }
}
